...
pub fn parse_config(content: &str) -> Result<HashMap<u64, GuildProtectionConfig>, String> {
    ...
    let guilds = value
        .as_object()
        .ok_or("La configuration doit être un objet par serveur")?;
    ...
    for (guild, value) in guilds {
        ...
        if let Some(unknown) = enabled
            .iter()
            .find(|name| !MODULES.contains(&name.as_str()))
        {
        ...
        if let Some(roles) = value.get("limited_roles") {
            for (role, limit) in roles
                .as_object()
                .ok_or("limited_roles doit être un objet")?
            {
        ...
        if config.blocked_words.len() > 1000
            || config
                .blocked_words
                .iter()
                .any(|word| word.chars().count() > 60)
        {
...
fn strings(value: Option<&Value>) -> Result<Vec<String>, String> {
    match value {
        ...
        Some(value) => value
            .as_array()
            .ok_or("Une liste est attendue")?
            .iter()
            .map(|v| {
                v.as_str()
                    .map(str::to_owned)
                    .ok_or("Une chaîne est attendue".into())
            })
            .collect(),
    }
}
...
fn ids(value: Option<&Value>) -> Result<HashSet<u64>, String> {
    match value {
        ...
        Some(value) => value
            .as_array()
            .ok_or("Une liste d'identifiants est attendue")?
            .iter()
            .map(snowflake)
            .collect(),
    }
...
fn snowflake(value: &Value) -> Result<u64, String> {
    value
        ...
        .filter(|id| *id != 0)
        .ok_or("Identifiant Discord invalide".into())
}
