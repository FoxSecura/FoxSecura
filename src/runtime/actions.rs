...
use super::{
    Action, Error, GuildProtectionConfig, PlannedAction, ProtectionRuntime, epoch, permissions_for,
};
use crate::{
    database::TemporarySlowmode,
    i18n::Language,
    logs::{
...
impl ProtectionRuntime {
    pub(super) async fn execute(
        ...
        for planned in actions {
            ...
            let result = if !config.enforce
                && !matches!(planned.action, Action::Alert | Action::SyncAutoMod)
            {
            ...
            if let Err(_) = self
                .log(ctx, guild, planned.module, code, status, user, failure)
                .await
            {
                eprintln!(
                    "FoxSecura : journal Discord indisponible, serveur {}, module {}",
                    guild, planned.module
                );
            }
        }
        Ok(())
        ...
        match action {
            ...
            Action::DeleteMessage {
                ...
                self.engine
                    ...
                    .discard_message(guild.get(), *message);
                ctx.http.delete_message(channel, current.id, reason).await?;
            }
            Action::Timeout { user }
            | Action::Kick { user }
            | Action::NormalizeNickname { user, .. }
            | Action::RemoveRole { user, .. }
            | Action::ContainExecutor { user } => {
                let Some((member, roles, bot_permissions, top)) =
                    self.action_member(ctx, guild, config, *user).await?
                else {
                    return Ok(ActionStatus::Skipped);
                };
                ...
                match action {
                    Action::Timeout { .. } => {
                        let member_roles: Vec<u64> =
                            member.roles.iter().map(|id| id.get()).collect();
                        if permissions_for(guild.get(), &member_roles, &roles)
                            .contains(serenity::Permissions::ADMINISTRATOR)
                        {
                            return Ok(ActionStatus::Skipped);
                        }
                        let deadline = serenity::Timestamp::from_unix_timestamp(
                            epoch().as_secs() as i64 + 600,
                        )?;
                        if member
                            .communication_disabled_until
                            .is_some_and(|until| until >= deadline)
                        {
                            return Ok(ActionStatus::Skipped);
                        }
                        ctx.http
                            .edit_member(
                                guild,
                                member.user.id,
                                &json!({"communication_disabled_until": deadline.to_string()}),
                                reason,
                            )
                            .await?;
                    }
                    ...
                    Action::NormalizeNickname { nickname, .. } => {
                        ...
                        ctx.http
                            .edit_member(guild, member.user.id, &json!({"nick": nickname}), reason)
                            .await?;
                    }
                    Action::RemoveRole { role, .. } => {
                        if !member.roles.iter().any(|id| id.get() == *role) {
                            return Ok(ActionStatus::Skipped);
                        }
                        let removable = roles.iter().any(|candidate| {
                            candidate.id.get() == *role
                                && !candidate.managed
                    ...
                    Action::ContainExecutor { .. } => {
                        ...
                        for role in roles.iter().filter(|role| {
                            ...
                            if ctx
                                .http
                                .remove_member_role(guild, member.user.id, role.id, reason)
                                .await
                                .is_ok()
                            {
                                succeeded += 1;
            ...
            Action::RemoveWebhook { webhook } => {
                let hook = ctx
                    .http
                    .get_webhook(serenity::WebhookId::new(*webhook))
                    .await?;
                if hook.guild_id != Some(guild) {
                    return Ok(ActionStatus::Skipped);
                }
                ctx.http.delete_webhook(hook.id, reason).await?;
            }
            Action::RollbackPermissions {
                role,
                old,
                expected,
            } => {
                let roles = ctx.http.get_guild_roles(guild).await?;
                let Some(current) = roles.iter().find(|candidate| candidate.id.get() == *role)
                else {
                    return Ok(ActionStatus::Skipped);
                };
                // Ne pas écraser une correction faite par un administrateur entre les deux événements.
                if current.managed
                    || current.id.get() == guild.get()
                    || current.permissions.bits() != *expected
                {
                    return Ok(ActionStatus::Skipped);
                }
                ctx.http
                    .edit_role(
                        guild,
                        current.id,
                        &json!({"permissions": old.to_string()}),
                        reason,
                    )
                    .await?;
            }
            Action::Slowmode { channel, seconds } => {
                return self
                    .apply_slowmode(ctx, guild, *channel, *seconds, 120)
                    .await;
            }
            Action::Lockdown => {
                let mut succeeded = 0;
                ...
                {
                    match self
                        .apply_slowmode(ctx, guild, channel.id.get(), 30, 900)
                        .await
                    {
    ...
    > {
        ...
        if user == owner || user == bot.get() {
            return Ok(None);
        }
        let member = ctx
            .http
            .get_member(guild, serenity::UserId::new(user))
            .await?;
        let member_roles: Vec<u64> = member.roles.iter().map(|id| id.get()).collect();
        if config.exempt(user, &member_roles) {
            return Ok(None);
        }
    ...
    async fn apply_slowmode(
        ...
        let mode = TemporarySlowmode {
            ...
            restore_at: epoch().as_secs() as i64 + duration,
            pending_seconds: Some(previous),
        };
        if !self.database.save_temporary_slowmode(&mode)? {
            return Ok(ActionStatus::Skipped);
        }
        ctx.http
            .edit_channel(
                ...
                Some("FoxSecura : ralentissement temporaire"),
            )
            .await?;
    ...
    pub(super) async fn restore_slowmodes(&self, ctx: &serenity::Context) -> Result<(), Error> {
        ...
        for mode in self.database.temporary_slowmodes()? {
            if mode.restore_at > epoch().as_secs() as i64 {
                continue;
            }
            let channel = match serenity::ChannelId::new(mode.channel)
                .to_channel(&ctx.http)
                .await
            {
            ...
            if let Some(channel) = channel {
                if channel.guild_id.get() != mode.guild {
                    continue;
                }
                    ...
                    || mode.pending_seconds == Some(channel.rate_limit_per_user.unwrap_or(0))
                {
                    if ctx
                        ...
                        .edit_channel(
                            channel.id,
                            &json!({"rate_limit_per_user": mode.previous_seconds}),
                            Some("FoxSecura : fin du ralentissement temporaire"),
                        )
                        .await
                        .is_err()
                    {
    ...
    pub(super) async fn log(
        ...
        let mut incident = SecurityIncident::new(
            ...
            LogType::Moderation,
            LogSeverity::Warning,
            format!("Protection {module} — serveur {guild}"),
            vec![SecurityActionOutcome {
                action: code,
                status,
                details: None,
        ...
        let channel = self
            .configs
            .get(&guild.get())
            .and_then(|config| config.log_channel)
            .or(self
                .database
                .log_channel(guild.get(), LogType::Moderation)?
        ...
        if let Some(channel) = channel {
            ...
            if channel
                ...
                .await?
                .guild()
                .is_none_or(|channel| channel.guild_id != guild)
            {
