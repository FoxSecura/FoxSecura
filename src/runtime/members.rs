...
impl ProtectionEngine {
    pub fn member(
        ...
        if joined && !self.admit(member.guild, member.user, "join", now) {
            return Vec::new();
        }
        ...
        if joined {
            ...
            let join_config = JoinBurstConfig {
                enabled: config.enabled("join_burst") && !member.profile_exempt,
                ..Default::default()
            };
            if self
                .joins
                ...
                == ProtectionDecision::Block
            {
                ...
                if !member.bot && !member.profile_exempt {
                    actions.push(PlannedAction::new(
                        "join_burst",
                        Action::Timeout { user: member.user },
                    ));
                }
            }
        }
        if !member.bot && !member.profile_exempt {
            let identities: Vec<AccountIdentity<'_>> = member
                .identities
                .iter()
                .map(|(id, name, avatar)| AccountIdentity::new(*id, Some(name), avatar.as_deref()))
                .collect();
            if config.enabled("anti_double_account")
                && detect_likely_double_account(AntiDoubleAccountInput {
                    user_id: member.user,
                    display_name: Some(&member.display_name),
                    avatar_hash: member.avatar.as_deref(),
            ...
            if config.enabled("anti_impersonation")
                && detect_impersonation(AntiImpersonationDetectionInput {
                    candidate_names: &[&member.display_name, &member.username],
            ...
            if config.enabled("anti_nickname_hoisting") {
                ...
                if result.triggered {
                    let nickname = if result.cleaned.is_empty() {
                        "Membre".into()
                    } else {
                        result.cleaned
                    };
        ...
        for &(role, before, after, count) in &member.role_counts {
            if should_enforce_limit_role(LimitRoleInput {
                enabled: config.enabled("limit_role"),
                role_configured: config.limited_roles.contains_key(&role),
                max_members: config.limited_roles.get(&role).copied(),
                had_role_before: before,
                has_role_now: after,
                current_role_member_count: count,
            }) {
                actions.push(PlannedAction::new(
                    "limit_role",
                    Action::RemoveRole {
