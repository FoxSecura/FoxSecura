...
use crate::{
    database::{DEFAULT_DATABASE_PATH, Database},
...
pub use messages::{MessageInput, MessageRevision};
...
impl ProtectionRuntime {
    pub fn from_env() -> Result<Self, Error> {
        ...
        let path = std::env::var("FOXSECURA_DATABASE_PATH")
            .unwrap_or_else(|_| DEFAULT_DATABASE_PATH.into());
        Ok(Self {
            configs,
            engine: Mutex::new(ProtectionEngine::default()),
            actions: tokio::sync::Mutex::new(()),
            database: Database::open(path)?,
            ai: ai::AiService::default(),
            provider: OpenAiModerationProvider::from_env(),
            started: Instant::now(),
        })
    }
    ...
    pub async fn handle(
        ...
        match event {
            serenity::FullEvent::Message { new_message } => {
                self.on_message(ctx, new_message, false).await?
            }
            serenity::FullEvent::MessageUpdate { new, event, .. } => {
                ...
                if event.content.is_some()
                    || event.attachments.is_some()
                    || event.mentions.is_some()
                    || event.mention_roles.is_some()
                    || event.mention_everyone.is_some()
                {
                    if let Some(guild) = event
                        .guild_id
                        .filter(|id| self.configs.contains_key(&id.get()))
                    {
                        ...
                        if message.guild_id == Some(guild) {
                            self.on_message(ctx, &message, true).await?;
                        }
                    }
                }
            }
            ...
            serenity::FullEvent::MessageDeleteBulk {
                ...
                self.on_deleted(ctx, *guild, multiple_deleted_messages_ids)
                    .await?;
            }
            ...
            serenity::FullEvent::GuildMemberUpdate {
                ...
                if self.configs.contains_key(&event.guild_id.get()) {
                    ...
                    self.on_member(ctx, &member, old_if_available.as_ref(), false)
                        .await?;
                }
            }
            ...
            serenity::FullEvent::GuildCreate { guild, .. } => {
                if let Some(config) = self.configs.get(&guild.id.get()) {
                    ...
                    self.execute(
                        ctx,
                        guild.id,
                        vec![PlannedAction::new("native_rules", Action::SyncAutoMod)],
                    )
                    .await?;
            ...
            serenity::FullEvent::AutoModActionExecution { execution } => {
                if self.configs.contains_key(&execution.guild_id.get()) {
                    self.log(
                        ...
                        execution.guild_id,
                        "native_rules",
                        crate::logs::ActionCode::RecordAlert,
                        crate::logs::ActionStatus::Success,
                        Some(execution.user_id.get()),
                        None,
                    )
                    .await?;
                }
    ...
    async fn on_message(
        ...
        let Some(config) = self.configs.get(&guild.get()) else {
            return Ok(());
        };
        let bot = ctx.cache.current_user().id;
        if message.author.id == bot || (message.author.bot && message.webhook_id.is_none()) {
            return Ok(());
        }
        ...
        let member = if message.webhook_id.is_none() {
            ...
            Some(match cached {
                Some(member) => member,
                None => ctx.http.get_member(guild, message.author.id).await?,
            })
        } else {
            None
        };
        let role_ids: Vec<u64> = member
            .as_ref()
            .map(|m| m.roles.iter().map(|id| id.get()).collect())
            .unwrap_or_default();
        ...
        let input = MessageInput {
            ...
            exempt: message.webhook_id.is_none()
                && (message.author.id.get() == owner
            ...
            webhook: message.webhook_id.map(|id| id.get()),
        ...
        let actions = self
            .engine
            .lock()
            .map_err(|_| "État de protection inaccessible")?
            .message(config, &input, self.started.elapsed(), epoch(), edited);
        let deleting = actions
            .iter()
            .any(|action| matches!(action.action, Action::DeleteMessage { .. }));
        self.execute(ctx, guild, actions).await?;
        if !deleting {
            let provider = self.provider.as_ref().map(|p| {
                p as &dyn crate::protection::ai_moderation::providers::AiModerationProvider
            });
            if let Some(plan) = self.ai.analyze(config, &input, provider, 5_000).await {
                let mut actions = Vec::new();
                if plan.log {
                    actions.push(PlannedAction::new("ai_moderation", Action::Alert));
                }
                        ...
                        if current.content == input.content {
                            actions.push(PlannedAction::new(
                                ...
                                Action::DeleteMessage {
                                    ...
                                    message: input.id,
                                    expected: MessageRevision::from(&input),
                                },
                            ));
                        }
                    }
    ...
    async fn on_deleted(
        ...
        let actions = {
            ...
            ids.iter()
                .flat_map(|id| {
                    engine.message_deleted(config, guild.get(), id.get(), self.started.elapsed())
                })
                .collect()
        };
        self.execute(ctx, guild, actions).await
    ...
    async fn on_member(
        ...
        let mut input = MemberInput {
            ...
            bot: member.user.bot,
            exempt: member.user.id.get() == owner
            ...
            if let Some(old) = old.filter(|_| cached.member_count as usize == cached.members.len())
            {
                input.role_counts = config
                    ...
                    .map(|role| {
                        ...
                        (
                            *role,
                            old.roles.contains(&id),
                            member.roles.contains(&id),
                            cached
                                .members
                                .values()
    ...
    async fn on_audit(
        ...
        if entry.user_id.get() == owner || entry.user_id.get() == bot {
            return Ok(());
        }
        ...
        let role = entry
            .target_id
            .and_then(|id| roles.iter().find(|role| role.id.get() == id.get()));
        let managed_rule = if matches!(entry.action, serenity::audit_log::Action::AutoMod(_)) {
            ...
            current.iter().any(|rule| {
                Some(rule.id.get()) == target
                    && rule.creator_id.get() == bot
                    && rule
                        .name
                        .starts_with(crate::protection::automod::native_rules::AUTOMOD_RULE_PREFIX)
            }) || target.is_some_and(|rule| {
                self.database
                    .is_managed_rule(guild.get(), rule)
                    .unwrap_or(false)
            })
        } else {
            false
        ...
        let context = audit::AuditContext {
            ...
            owner,
            bot,
            executor_resolved: member.is_some(),
            executor_exempt: config.exempt(entry.user_id.get(), &role_ids),
            role_managed: role.is_none_or(|role| role.managed),
            managed_rule,
        };
        let actions = self
            .engine
    ...
    async fn guild_identity(
        &self,
        ctx: &serenity::Context,
        guild: serenity::GuildId,
    ) -> Result<(u64, Vec<serenity::Role>), Error> {
        if let Some(cached) = ctx.cache.guild(guild) {
            return Ok((
                cached.owner_id.get(),
                cached.roles.values().cloned().collect(),
            ));
        }
        let guild = guild.to_partial_guild(&ctx.http).await?;
        Ok((
            guild.owner_id.get(),
            guild.roles.values().cloned().collect(),
        ))
    }
}
...
fn permissions_for(
    ...
    roles
        .iter()
        .filter(|role| role.id.get() == guild || role_ids.contains(&role.id.get()))
        .fold(serenity::Permissions::empty(), |permissions, role| {
