...
use crate::protection::{
    anti_nuke::{
        ...
        member_actions::{
            anti_mass_ban::detect_mass_ban, anti_mass_kick::detect_mass_kick,
            anti_mass_timeout::detect_mass_timeout, anti_mass_unban::detect_mass_unban,
        },
        resource_actions::{
            anti_channel_delete::detect_channel_delete,
            ...
            anti_mass_role_grant::detect_mass_role_grant, anti_role_delete::detect_role_delete,
        },
        server_integrity::{
...
use poise::serenity_prelude::{
    Permissions,
    audit_log::{
        Action as AuditAction, AuditLogEntry, AutoModAction, Change, ChannelAction, MemberAction,
        RoleAction, WebhookAction,
    },
};
...
impl ProtectionEngine {
    pub fn audit(
        ...
        if !self.admit(context.guild, entry.id.get(), "audit", now) {
            return Vec::new();
        }
        let executor = entry.user_id.get();
        let exempt =
            context.executor_exempt || executor == context.owner || executor == context.bot;
        if exempt {
            return Vec::new();
        }
        ...
        let burst = match entry.action {
            ...
            AuditAction::Member(MemberAction::Update)
                if enabled("anti_mass_timeout")
                    && changes.iter().any(|change| {
                        matches!(change, Change::CommunicationDisabledUntil {
                    old, new: Some(new),
                    ...
                    }) =>
            {
                Some((
                    "anti_mass_timeout",
                    detect_mass_timeout(&mut self.burst, input),
                ))
            }
            AuditAction::Member(MemberAction::RoleUpdate)
                if enabled("anti_mass_role_grant")
                    && changes.iter().any(|change| {
                        matches!(change, Change::RolesAdded {
                    new: Some(roles), ..
                } if !roles.is_empty())
                    }) =>
            {
                Some((
                    ...
                    detect_mass_role_grant(&mut self.burst, input),
                ))
            }
            AuditAction::Channel(ChannelAction::Create) if enabled("anti_mass_channel_create") => {
                Some((
                    "anti_mass_channel_create",
                    detect_mass_channel_create(&mut self.burst, input),
                ))
            }
            AuditAction::Role(RoleAction::Create) if enabled("anti_mass_role_create") => Some((
                "anti_mass_role_create",
                detect_mass_role_create(&mut self.burst, input),
            )),
            AuditAction::Emoji(_) | AuditAction::Sticker(_)
                if enabled("anti_emoji_sticker_nuke") =>
            {
                Some((
                    "anti_emoji_sticker_nuke",
                    detect_emoji_sticker_nuke(&mut self.burst, input),
                ))
            }
            _ => None,
        };
        ...
        if matches!(entry.action, AuditAction::Channel(ChannelAction::Delete))
            && detect_channel_delete(
                enabled("anti_channel_delete"),
                context.executor_resolved,
                exempt,
        ...
        if matches!(entry.action, AuditAction::Role(RoleAction::Update)) {
            for change in changes {
                ...
                } = change
                {
                    ...
                    if detect_permission_escalation(
                        enabled("anti_permissions"),
                        *old,
                        *new,
                        role == context.guild,
                        context.role_managed,
                        context.executor_resolved,
                        exempt,
                    ) != ExecutorDecision::Ignore
                    {
                    ...
                    let external =
                        detect_external_application_permission_added(ExternalApplicationInput {
                            enabled: enabled("anti_external_application"),
                            old_permissions: old.bits(),
                            new_permissions: new.bits(),
                            permission_flag: Some(1u64 << 50),
                            role_is_in_flight: false,
                            role_is_everyone: role == context.guild,
                            role_is_managed: context.role_managed,
                            executor_id: context.executor_resolved.then_some(executor),
                            executor_is_owner: executor == context.owner,
                            executor_is_whitelisted: exempt,
                        });
        ...
        if matches!(entry.action, AuditAction::GuildUpdate) {
            let fields: Vec<ServerField> = changes
                .iter()
                .filter_map(|change| match change {
                    ...
                    Change::DefaultMessageNotifications { .. } => {
                        Some(ServerField::DefaultNotifications)
                    }
                    Change::ExplicitContentFilter { .. } => {
                        Some(ServerField::ExplicitContentFilter)
                    }
                ...
                .collect();
            if detect_server_edit(
                enabled("anti_server_edit"),
                &fields,
                context.executor_resolved,
                exempt,
            )
            .decision
                != ExecutorDecision::Ignore
            {
                modules.push("anti_server_edit");
            }
            for change in changes {
                if let Change::VanityUrlCode { old, new } = change {
                    if detect_vanity_change(
                        enabled("anti_vanity_change"),
                        old.as_deref(),
                        new.as_deref(),
                        context.executor_resolved,
                        exempt,
        ...
        if let Some(change) = rule_change {
            if detect_managed_rule_change(
                enabled("automod_rule_guard"),
                context.managed_rule,
                change,
                context.executor_resolved,
                exempt,
            ) != ExecutorDecision::Ignore
        ...
        if enabled("webhook_watch") && matches!(entry.action, AuditAction::Webhook(_)) {
            if !context.executor_resolved {
            ...
            } else if is_unauthorized_webhook_executor(
                Some(executor),
                Some(context.bot),
                context.owner,
                exempt,
                ...
                if matches!(
                    entry.action,
                    AuditAction::Webhook(WebhookAction::Create | WebhookAction::Update)
                ) {
        ...
        for module in &modules {
            ...
            if context.executor_resolved
                && enabled("panic_mode")
                && self.panic.record(context.guild, *module, now).triggered
            {
        ...
        if let Some(module) = modules.first() {
            if context.executor_resolved {
                actions.push(PlannedAction::new(
                    module,
                    Action::ContainExecutor { user: executor },
                ));
            }
        }
        actions
