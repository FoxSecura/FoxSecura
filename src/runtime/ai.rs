...
use crate::protection::ai_moderation::{
    ...
    providers::{
        AiCompletionRequest, AiCompletionResult, AiModerationProvider, AiProviderDescriptor,
    },
...
impl Default for AiService {
    fn default() -> Self {
        Self {
            state: Mutex::new(AiModerationRuntime::default()),
            started: Instant::now(),
        }
    }
}
...
impl AiService {
    pub async fn analyze(
        ...
        if !config.enabled("ai_moderation")
            || message.exempt
            || message.bot
            || message.webhook.is_some()
        ...
        let settings = AiModerationSettings {
            enabled: true,
            timeout_ms,
            ..Default::default()
        };
        let request = AiAnalysisRequest {
            guild_id: message.guild.to_string(),
            channel_id: message.channel.to_string(),
            revision_id: None,
            message: AiMessageSnapshot {
                message_id: message.id.to_string(),
                author_id: message.author.to_string(),
                author_label: "AUTHOR".into(),
        ...
        let outcome = match prepared {
            ...
            AnalysisPreparation::Ready { key, input } => {
                let completion = tokio::time::timeout(
                    ...
                    provider.complete(AiCompletionRequest {
                        input: input.input,
                        timeout_ms,
                    }),
                )
                ...
                let completion = completion.unwrap_or_else(|_| AiCompletionResult::Error {
                    reason: AiFailureReason::Timeout,
            ...
            AnalysisPreparation::JoinInFlight { .. } | AnalysisPreparation::Skipped(_) => {
                return None;
            }
        };
        match outcome {
            AiAnalysisOutcome::Classified { classification, .. } => Some(plan_action(
                &decide_ai_moderation_action(&classification, &settings.rule_settings()),
            )),
