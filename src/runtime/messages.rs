...
use crate::protection::{
    anti_raid::{
        ...
        webhook_message::{
            WebhookMessageDetectionInput, WebhookMessageSpamInput, detect_webhook_message_threat,
        },
    },
    anti_spam::{
    ...
    automod::{
        adult_link::detect_adult_link,
        anti_invite::detect_invite_link,
...
/// État analysé : une correction du texte ou des pièces jointes annule l'action en attente.
...
impl From<&MessageInput> for MessageRevision {
    fn from(message: &MessageInput) -> Self {
        Self {
            content: message.content.clone(),
            attachments: message.attachments.clone(),
            mentions: message.mentions.clone(),
            role_mentions: message.role_mentions.clone(),
            mentions_everyone: message.mentions_everyone,
        }
    }
}

impl From<&poise::serenity_prelude::Message> for MessageRevision {
...
impl MessageInput {
    fn ghost(&self, now: Duration) -> GhostPingMessage {
        GhostPingMessage {
            ...
            mention_ids: self.mentions.clone(),
            role_mention_ids: self.role_mentions.clone(),
...
impl ProtectionEngine {
    pub fn message(
        ...
        if message.guild == 0
            || message.exempt
            || config.ignored_channels.contains(&message.channel)
        ...
        let links = MaliciousLinkContext {
            now: Some(epoch),
            account_created_at: message.created_at,
            member_joined_at: message.joined_at,
            ..Default::default()
        };
        for (module, triggered) in [
            (
                "anti_everyone",
                enabled("anti_everyone")
                    && detect_everyone_mention(&message.content, Some(message.mentions_everyone))
                        .triggered,
            ),
            (
                "anti_mass_mention",
                enabled("anti_mass_mention") && detect_mass_mention(mentions, None).triggered,
            ),
            (
                "attachment_filter",
                enabled("attachment_filter") && detect_dangerous_attachment(&filenames).triggered,
            ),
            (
                "invisible_char_filter",
                enabled("invisible_char_filter")
                    && detect_obfuscated_text(&message.content).triggered,
            ),
            (
                "malicious_link",
                enabled("malicious_link")
        ...
        if enabled("honeypot")
            && detect_honeypot_message(HoneypotDetectionInput {
                channel_id: message.channel,
                honeypot_channel_id: config.honeypot_channel,
                is_owner: message.author == message.owner,
                is_administrator: false,
                can_manage_guild: false,
                whitelisted: message.exempt,
            })
            .triggered
        {
        ...
        if !edited {
            if enabled("message_flood") {
                let flood = MessageFloodConfig {
                    enabled: true,
                    ..Default::default()
                };
                let count = self
                    .burst
                    .detect(ActionBurstInput::new(
                        message.guild,
                        message.author,
                        "message_flood",
                        flood.message_limit + 1,
                        flood.window,
                        now,
                    ))
                    .count;
                if evaluate(flood, MessageWindow::new(count, Duration::ZERO))
                    == ProtectionDecision::Block
                {
                    reasons.push("message_flood");
                }
            }
            if enabled("anti_ping_owner")
                && self
                    .owner_ping
                    .detect(AntiPingOwnerInput {
                        enabled: true,
                        author_is_bot: message.bot,
                        author_is_owner: message.author == message.owner,
                        mentions_owner: message.mentions.contains(&message.owner),
            ...
            let targets: Vec<MentionTarget> = message
                .mentions
                .iter()
                .map(|id| MentionTarget {
                    kind: MentionTargetKind::User,
                    id: *id,
                })
                .chain(message.role_mentions.iter().map(|id| MentionTarget {
                    kind: MentionTargetKind::Role,
                    id: *id,
                }))
                .collect();
            if enabled("anti_spam_ping")
                && self
                    .spam_ping
            ...
            if enabled("auto_slowmode")
                && self
                    .slowmode
                    .record(message.guild, message.channel, now)
                    .decision
                    == ProtectionDecision::Block
            {
        ...
        if let Some(webhook) = message.webhook.filter(|_| enabled("webhook_message")) {
            let count = if edited {
                ...
                self.webhook_spam
                    .detect(WebhookMessageSpamInput {
                        guild_id: message.guild,
                        webhook_id: webhook,
                        timestamp: now,
                        threshold: None,
            ...
            if detect_webhook_message_threat(WebhookMessageDetectionInput {
                content: &message.content,
                mentions_everyone: Some(message.mentions_everyone),
                mention_count: mentions,
                mass_mention_threshold: None,
                recent_message_count: Some(count),
                spam_threshold: None,
                malicious_link_context: links,
            })
            .triggered
            {
        ...
        if enabled("anti_ghost_ping") && message.webhook.is_none() {
            if edited {
                ...
                self.ghost.record(message.ghost(now));
            }
        }
        if let Some(module) = reasons.first().copied() {
            actions.push(PlannedAction::new(
                ...
                Action::DeleteMessage {
                    channel: message.channel,
                    message: message.id,
                    expected: MessageRevision::from(message),
                },
            ));
            ...
            actions.extend(
                reasons
                    .into_iter()
                    .skip(1)
                    .map(|module| PlannedAction::new(module, Action::Alert)),
            );
        }
        actions
    }
    ...
    pub fn message_deleted(
        ...
        if !config.enabled("anti_ghost_ping") {
            return Vec::new();
        }
